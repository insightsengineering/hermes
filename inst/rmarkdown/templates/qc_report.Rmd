...
params:
  ...
  author: 
    ...
    value: "Firstname Lastname (PDD - Data Science Acceleration (DSX))"
    input: text
  input_data_object:
...
```{r}
object_tg <- top_genes(object, n_top = params$top_gene_n_top, min_threshold = params$top_gene_min_threshold)
```
...
```{r}
object_sds <- top_genes(object, summary_fun = rowSds, n_top = params$top_gene_n_top, min_threshold = params$top_gene_min_threshold)
...
```{r}
object_pca <- calc_pca(object, assay_name = params$assay_name)
...
```{r}
object_pca_normalized <- calc_pca(object_normalized, assay_name = params$assay_name)
...
```{r}
object_pca_filtered <- calc_pca(object_normalized_filtered, assay_name = params$assay_name)
...
### Comparison of default technical failure with customized setting

Next we list the samples where the default technical failure flags are different from the resulting ones, using
the custom correlation score threshold:
...
Next we list the samples where the default low depth flags are different from the resulting ones, using
the custom depth threshold:
...
```{r}
df_comp_normalization <- data.frame(
  default = get_low_expression(object_normalized),
  result = get_low_expression(object)
)
DT::datatable(subset(df_comp_normalization, default != result))
```
