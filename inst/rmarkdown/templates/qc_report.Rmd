...
# Load Data, Filter Genes/Samples and Normalize
...
### Comparison of default flags with resulting flags
...
subset(df_comp_tech_failure, default != result)
```

## Low depth samples 

Identified by lower quartile minus 1.5*IQR (default) or user-defined threshold: <b> 
`r ifelse(is.null(params$min_depth), "still default", params$min_depth)` </b> 
...
subset(df_comp_low_depth, default != result)
```
...
```{r}
# added hermes:: because couldn't find function when executing 
result_cor <- hermes::calc_cor(result, method = params$cor_method) 
autoplot(result_cor)
```
{-}
...
## Technical Metrics After Normalization and Gene Filtering {.tabset}

### Histogram of Library Sizes 

```{r , results='asis'}
draw_libsize_hist(result_fil)
```

### Q-Q Plot of Library Sizes 

```{r , results = 'asis'}
draw_libsize_qq(result_fil)
```

### Density Plot of (Log) Counts Distributions 

```{r , results = "asis"}
draw_libsize_densities(result_fil)
```

{-}

## Principal Components Analysis Comparison{.tabset}

### PCA after Normalization
```{r}
result_pca_norm <- calc_pca(result_norm, assay_name = params$assay_name)
autoplot(result_pca_norm, label = TRUE)
```

### PCA after QC and Normalization 

```{r}
result_pca_fil <- calc_pca(result_fil, assay_name = params$assay_name)
autoplot(result_pca_fil, label = TRUE)
```

{-}

### Count distribution post normalization
