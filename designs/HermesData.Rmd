...
The `HermesData` object has similarities from the `SummarizedExperiment` and the `gSummarizedExperiment` objects. To be specific,

* The first assay must be `counts` containing non-missing, integer, non-negative values.

* Similar to the `gSummarizedExperiment()`, `HermesData()` must have mandatory rowData (HGNC, HGNCGeneName, etc) and colData (Filename, SampleID, etc). 
