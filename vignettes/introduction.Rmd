...
```{r}
object <- hermes_data # REPHRASE SENTENCE
```
...
achieved with the `normalize()` function. The `normalize()` function can also transform the counts by the variance stabilizing transformation (VST) and the regularized log transformation (rlog). 
