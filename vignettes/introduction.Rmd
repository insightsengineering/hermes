...
We can load the prepared `hermes_data` object, which is constructed out of the SE using the `HermesData()` constructor:

```{r}
object <- hermes_data # HermesData(summarized_experiment)
```
