...
`hermes` is a successor of the [`rnaseqTools`](https://pages.github.roche.com/Rpackages/rnaseqTools/) R package, and therefore many code pieces and ideas have been borrowed from it. We therefore would like to thank the `rnaseqTools` authors for their work. In particular, we would like to acknowledge Chendi Liao and Joe Paulson for their guidance and explanations during the development of `hermes`. We also discussed the class design with Valerie Obenchain, and discussed RNAseq data standards with Armen Karapetyan. We borrowed some ideas from [`biokitr`](https://pages.github.roche.com/BEDA/biokitr/) and discussed with its maintainer Daniel Marbach.
...
Note that in the next release we plan to separate the internal IGIS part from the 
general annotation framework and open source (`biomaRt` based) part to allow open 
sourcing of the largest `hermes` parts.

### Connection to Database

The first step is to connect to a database. Currently, the only option is IGIS.
The corresponding function takes the common gene ID prefix as argument to internally
...
Here we are using the `genes()` method to access the gene IDs (row names) of the `HermesData` object. Note that actual usage might give warnings from the `igis` package here, which we will work on fixing soon (outside of NEST).
