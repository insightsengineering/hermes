...
setMethod(
  ...
  definition = function(x, map_names) {
    assert_list(map_names, types = "character")
    assert_subset(names(map_names), c("row_data", "col_data", "assay"))
    
    if (!is.null(row_map <- map_names$row_data)) {
      row_data <- rowData(x)
      col_names <- names(row_data)
      col_pos <- h_map_pos(names = col_names, map = row_map)
      names(rowData(x))[col_pos] <- names(row_map)
    }
    if (!is.null(col_map <- map_names$col_data)) {
      col_data <- colData(x)
      col_names <- names(col_data)
      col_pos <- h_map_pos(names = col_names, map = col_map)
      names(colData(x))[col_pos] <- names(col_map)
    }
    if (!is.null(assay_map <- map_names$assay)) {
...
object2 <- rename(object, map_names = list(
  row_data = c(symbol = "HGNC"), 
  col_data = c(low_depth_flag = "LowDepthFlag"),
  assay = c(counts = "count")
))
