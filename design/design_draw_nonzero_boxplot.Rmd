...
draw_nonzero_boxplot <- function(object, jitter = 0.2, alpha = 1/4, ...){
    ...
    df <- data.frame(noNA_count)
                     
    df$x <- "Sample"
    ggplot(df, aes(y = .data$noNA_count, x = x)) +
...
```
## Question
In the example the oulier are labeled, here are not. Should I modify taking out the geom_point and leaving outlier option with label instead? not sure if it is possible to do both.
## Answer
This could be  because count will be hidden by the same named function in dplyr. Better use sum here instead.
