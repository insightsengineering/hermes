...
- first create, add flags, filter HermesData object
...
diff_expression <- function(object,
  ...
  assert_that(
    ...
    is.string(group),
    group %in% colnames(colData(object)), ## tern::is_df_with_nlevels_factor does not work
    length(unique(colData(object)[[group]])) == 2L, ## tern::is_df_with_nlevels_factor does not work
    is.string(method), 
    method %in% c("limma_voom", "deseq2")
  )
