...
So now putting the two functions togheter , need also to add an option to specify either 1 or the other option. I think with `FALSE` should be ok . 
adding also another function call that the summary function can be choose even if the default is rowmean
...
top_genes <- function(object = result,
  ...
  assert_that(
    ...
    # QUESTION  which assert function can I use when are either positive and numeric or NA? 
    # how to assert that only one of the two is numeric? I added a print error but not sure if it is enough
    )
  ...
  # QUESTION: instead of using a variable name average expression should I make it more generic? is there a way that it reflect the summary function used?? 
  
  average_expression <- summary_fun(counts(object))
  average_expression <- as.data.frame(average_expression) %>%
    arrange(desc(average_expression))
