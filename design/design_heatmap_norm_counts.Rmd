...
result <- hermes_data %>%
  ...
  add_quality_flags() %>%
  # subset(subset = !low_expression_flag) %>%
  assays()
...
NormalizedHeatmap <- function(datasetInput,
  ...
  if (remove_low_expression){
    notLowVector <- rowSums(filteredData)
    notLowVector <- notLowVector > mean(notLowVector)
    filteredData <- filteredData[notLowVector,]
  }
  
  if (remove_zero_expression){
    notZeroVector <- rowSums(filteredData)
    notZeroVector <- notZeroVector > 0
    filteredData <- filteredData[notZeroVector,]
  }
